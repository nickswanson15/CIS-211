...
Nick Swanson
Appointments
Creates 2 python classes appt, which will record dates and times which the appoints are, and agenda, and reads a  text file.
'''
...
class Appt:
    ...
    def intersect(self, other: 'Appt') -> 'Appt':
        ...
        return (min(self.finish, other.finish)) - (max(self.start, other.start))
    ...
    def conflicts(self) -> 'Agenda':
        ...
        for i in self.elements:
            for e in self.elements:
                if self.elements[e].start < self.elements[i].finish:
                    x += 1
                    e += 1
                else:
                    break
